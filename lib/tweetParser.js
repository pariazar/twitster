...
exports.extractTweetsInfo = async (data, lang) => {
  ...
  for (let i = 1; i <= userListSize; i++) {
    ...
    const galleryRow = dom.window.document.querySelectorAll(`body > div > div > div.timeline > div:nth-child(${String(i)}) > div div.attachments`);
    for (let imageElement of galleryRow) {
      const imageResult = imageElement.querySelector('div > div > a > img');
      if(imageResult) images.push('https://nitter.tiekoetter.com'+imageResult.src);
    }
    ...
    let id;
    if (tweetLink && tweetLink.href) id = String(cryptoJs.MD5(tweetLink.href)).substr(4, 10);
    ...
    if (lang && tweetResult.tweetContent) tweetContents.push(tweetResult.tweetContent + ' 77*^66 ' + id);
    tweets.push(tweetResult);
  }
  for (const tweet of tweets) Object.keys(tweet).forEach(key => { tweet[key] === undefined ? delete tweet[key] : {} });
  if (lang) {
    const translatedTweets = await translate(tweetContents.join(' *** '), { to: lang });
    const transTweetsArr = translatedTweets.text.split(' *** ');
    for (let i = 0; i < transTweetsArr.length; i++) {
      if (transTweetsArr[i] && tweets[i] && tweets[i].tweetContent) {
        if (transTweetsArr[i].split(' 77*^66 ')[1] === tweets[i].id) {
          tweets[i].originalTweet = tweets[i].tweetContent
          tweets[i].tweetContent = transTweetsArr[i].split(' 77*^66 ')[0];
        }
      }
    }
  }
...
exports.extractUserDetail = async (data) => {
  ...
  for (let i = 1; i < userListSize; i++) {
    ...
    const galleryRows = dom.window.document.querySelectorAll(`body > div > div > div.timeline-container > div > div:nth-child(${String(i)}) > div > div.attachments`);
    for (let imageElement of galleryRows) {
      const imageResult = imageElement.querySelector('div > div > a > img');
      if(imageResult) images.push('https://nitter.tiekoetter.com'+imageResult.src);
    }
